...
def create_problem(mesh, ft):
    ...
    V_element = ufl.VectorElement("CG", mesh.ufl_cell(), 1)
    V = fem.FunctionSpace(mesh, V_element)
    ...
    for marker, val in zip([2, 3, 4, 5], [0, 0, 0, 2]):
        ...
        bc = fem.dirichletbc(fem.Constant(mesh, PETSc.ScalarType((val, -val))), dofs, V)
        bcs.append(bc)

    u = ufl.TrialFunction(V)
    v = ufl.TestFunction(V)

    f = fem.Constant(mesh, PETSc.ScalarType((1.0, 1.0)))
...
def main():
    ...
    with io.XDMFFile(mesh.comm, "output/ex02_uh_vLtest.xdmf", "w") as xdmf:
